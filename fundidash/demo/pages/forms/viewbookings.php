...
if(isset($_GET['type']) && $_GET['type']!=''){
  ...
  if($type=='status'){
    ...
    $update_status_sql="update bookings set status='$status' where id='$id'";
...
$sql="SELECT * FROM bookings WHERE fundiname = '{$_SESSION['fundi_name']}'";
$res=mysqli_query($conn,$sql);

$served = "SELECT * FROM BOOKINGS WHERE fundiname = '{$_SESSION['fundi_name']}' && status = 0";
$done = mysqli_query($conn,$served);
            ...
            <div class="mdc-layout-grid__inner">
              ...
              <tr style="background-color:white; font-size: 14px;">
                 ...
                 <td>
                   <?php
                if($row['status']==1){
                  echo "<a class='btn btn-sm btn-danger' href='?type=status&operation=deactive&id=".$row['id']."'>Pending</a>&nbsp;";
                }else{
                  echo "<a class='btn btn-sm btn-success' href='?type=status&operation=active&id=".$row['id']."'>served</a>&nbsp;";
                }
                ?>
              </td>
              <td>
              ...
              <div class="mdc-layout-grid__cell stretch-card mdc-layout-grid__cell--span-12">
                <div class="mdc-card p-0">
                  <h6 class="card-title card-padding pb-0">Served</h6>
                  <div class="table-responsive">
                    <table class="table">
                      <thead>
                        <tr>
